...
function shell(title: string, body: string): string {
  return `<!doctype html><html><body style="font-family:-apple-system,Segoe UI,Roboto,Helvetica,Arial,sans-serif;background:#f5f5f0;margin:0;padding:24px;color:#111">
    <div style="max-width:560px;margin:0 auto;background:#fff;border-radius:12px;overflow:hidden;border:1px solid #e5e7eb">
      <div style="background:#0b3d2e;color:#fff;padding:16px 24px;font-weight:700;font-size:18px">🏈 Jim Olah Survivor Pool — ${title}</div>
...
export function weekSummaryEmail(d: WeekSummaryData) {
  ...
  return {
    subject: `Week ${d.week} results — ${d.year} Jim Olah Survivor Pool`,
...
export function magicLinkEmail(url: string) {
  const body = `
    <p>Click below to sign in to the Jim Olah Survivor Pool. This link expires shortly and can only be used once.</p>
